...
export default class AddSeats extends Component {
    ...
    componentDidMount() {
        const traintid = this.props.match.params.tid;
        if(traintid) {
            this.findSeatById(traintid);
        }
    }   

    findSeatById = (traintid) => {
        axios.get("http://localhost:8080/api/seats/"+traintid)
            .then(response => {
	...
	submitSeat = event => {
        ...
        axios.post("http://localhost:8080/api/seats/"+seat.tid, seat)
            .then(response => {
                if(response.data != null) {
    ...
    updateSeat = event => {
        ...
        axios.put("http://localhost:8080/api/seats/"+this.state.tid, seat)
            .then(response => {
                if(response.data != null) {
                        ...
                        <Form.Row>
                            <Form.Group as={Col} controlId="formGridDate">
                                ...
                                <Form.Control required
                                    ...
                                    onChange={this.seatChange}
                                    className={"bg-dark text-white"}
                                    placeholder="Enter Train-Seat Date" />
                            </Form.Group>
                        </Form.Row>
                        <Form.Row>
